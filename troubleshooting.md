...
This troubleshooting guide contains a list of common issues and solutions for Podman.
...
When doing a `podman pull` or `podman build` command and an image cannot be pulled,
...
---
## Rootless: could not get runtime - database configuration mismatch
...
Be sure the UID (numerical user ID) of the user is part of the range in the
...
`/etc/sysctl.d` that contains `net.ipv4.ping_group_range=0 2000000`.

Specific UID limits are found in `/etc/login.defs`.
